...
plot_condition <- function(annualCondition,filename= "test1",out.dir = "output") {
  #turn off if running in function:
  annualCondition <- condMAB
  
  speciesNames <- annualCondition %>% dplyr::filter(sex == "F") %>%
    dplyr::select(Species) %>%
    dplyr::distinct(Species) %>% as.numeric(YEAR)
  speciesNames <- speciesNames$Species
  print(speciesNames)
  
  speciesNames <- as.numeric(YEAR)
  
  #Sort species names by highest to lowest condition averaged over the last 5 years
  sortNames <- speciesNames %>% 
    filter(YEAR == c("2015", "2016", "2017", "2018", "2019")) %>%
    group_by(Species) %>% 
...
#See 5 (or nQuants number of) scale colors for viridis:
     #scales::show_col(viridis::viridis_pal()(5))
     vir <- viridis::viridis_pal()(nQuants)
...
#   viridis_pal(option = "D")(5) 
   graph.colors <- colorRampPalette(vir)
